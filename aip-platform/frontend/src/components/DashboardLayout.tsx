...
const teamNavItems: NavItem[] = [
    { to: '/team', label: 'Team Dashboard', icon: <TeamDashboardIcon /> },
    { to: '/team/members', label: 'Members', icon: <TeamMembersIcon /> },
    { to: '/team/candidates', label: 'Candidates', icon: <TeamCandidatesIcon /> },
    { to: '/team/analytics', label: 'Analytics', icon: <TeamAnalyticsIcon /> },
    { to: '/team/resumes', label: 'Resume Bank', icon: <TeamResumesIcon /> },
    { to: '/team/settings', label: 'Settings', icon: <TeamSettingsIcon /> },
];
    ...
    const SidebarContent = ({ isCollapsed = false }: { isCollapsed?: boolean }) => (
        <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
            ...
            <nav style={{ flex: 1, padding: isCollapsed ? '8px 8px' : '16px 12px', overflowY: 'auto' }}>
                ...
                {navItems.map(item => {
                    const active = location.pathname.startsWith(item.to) && (item.to !== '/analyzer' || location.pathname === '/analyzer');
                    return (
                        <Link
                            key={item.to}
                            to={item.to}
                ...
                {user?.user_metadata?.isEnterprise && (
                    <div style={{ marginTop: 24 }}>
                        ...
                        {isCollapsed && <div style={{ height: 1, background: '#e8f5f1', margin: '8px 12px 16px' }} />}
                        ...
                        {teamNavItems.map(item => {
                            ...
                            return (
                                <Link
                                    ...
                                    onMouseLeave={e => { if (!active) (e.currentTarget as HTMLElement).style.background = 'transparent'; }}
                                >
                                    <span style={{ color: active ? 'var(--heading)' : '#7a9e97', flexShrink: 0 }}>{item.icon}</span>
                                    {!isCollapsed && item.label}
                                </Link>
                            );
                        })}
